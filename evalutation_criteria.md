...
Your submission for the **HackerRank Buildathon** is evaluated across these dimensions. Each dimension is scored independently and then combined into your final score.
...
A 30-minute interview with the AI Judge (camera on, mandatory).
...
We look at the UI you built to show tickets being processed.

We look for:

- **Functionality** — the UI actually runs and visibly processes tickets (queue, current ticket, decision, response), not a mockup or a static screen.
- **Visual polish** — a clean, considered interface rather than a bare default.
- **Clarity** — the queue, the current ticket, the decision, and the response are easy to follow at a glance.
- **Usability** — sensible layout and flow that someone could watch and understand without explanation.
